...
headers.append(ths[3].text.rstrip().split('[')[0])
...
head1 = f'{headers[0]}{pattern1}|{pattern1}{headers[1]}{pattern1}|{pattern1}{headers[2]}{pattern1}|{pattern1}{headers[3]}{pattern1}'
...
for i in range(2, len(trs)):
    ...
    if len(tds) == 19:
        name = tds[0].a.string
        nationality = tds[2].a["title"]
        num = tds[1].string
        seasons = tds[3].text
        #  print(f'{name} | {nationality} | {num}  | {seasons}')
        data = f'{name} | {num} | {nationality}| {seasons}'
        w_file.write(data + '\n')
        total_tenderer += 1
...
pattern1 = ' '*5
# print(f'{headers[0]}{pattern1}|{pattern1}{headers[1]}{pattern1}|{pattern1}{headers[2]}')
head2 = f'{headers[0]}{pattern1}|{pattern1}{headers[1]}{pattern1}|{pattern1}{headers[2]}{pattern1}|{pattern1}{headers[3]}'
w_file.write(head2 + '\n')
