...
headers.append(ths[2].text.rstrip().split('[')[0])
...
for i in range(2, len(trs)):
    ...
    if len(tds) and len(th):

        name = th.a.string
        nationality = tds[0].a['title'].rstrip()
...
for i in range(2, len(trs)):
    ...
    if len(tds) and len(th):
        name = th.a.string
        nationality = tds[0].a['title']
        seasons = tds[2].text.strip()
        # print(f'{name} | {nationality} | {seasons}')
        data = f'{name} | {nationality} | {seasons}'
        w_file.write(data + '\n')
        total_skater += 1
